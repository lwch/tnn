...
func (layer *SelfAttention) Forward(q, k, v, mask *tensor.Tensor, train bool) *tensor.Tensor {
	...
	if layer.wv == nil {
		layer.wv = layer.initW(lastDim(v), lastDim(v))
	}
	if layer.bq == nil {
	...
	y = y.Reshape(outputShape...)       // (batch, steps, dims)
	return y
}
