...
func TestSelfAttention(t *testing.T) {
	input := tensor.New([]float32{1, 2, 3, 4, 5, 6}, 2, 3)
...
func TestSelfAttentionCustom(t *testing.T) {
	...
	fmt.Println(mat32.Formatted(y.Grad().Value()))
}
...
func TestSelfAttentionRun(t *testing.T) {
	...
	y.SetRequireGrad(true)
	w := tensor.New([]float32{-0.0068, -0.5668, 0.3913,
	...
	score2 := score1.Scale(1 / float32(math.Sqrt(3)))
